...
- **Desestructurar Objetos:**
	...
	const {name, email: correo, ...restoPropiedades} = user; // 👈 Solo seleccionamos las propiedades que deseamos

	console.log(correo); // 👈 Email ha sido renombrado por correo
...
- **Operador de asignacion OR**
	- Si el valor es *falsy* asignara lo de la derecha
	```js

	// 👇 Si el array children es falsy, asignara 0
	const childrensQuantity = person.children?.lenght || 0;
	```
...
- **Truthy y Falsy** 
	...
	// 👇 Se ejecuta lo de al lado
	isLoggedIn && console.log(`Hola, Usuario!`);


	```
	- ✅ `!!token` → cuando **quieres saber si el valor "existe" (es truthy)**.
	- ❗ `!token` → cuando **quieres saber si el valor "no existe" (es falsy)**.
